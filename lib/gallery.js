...
import { getLikedIdSet, setLikeId } from "./likes-store.js";
import { ensureWorkId, readWorkId } from "./work-id.js";
...
const GROUP_SIZE = 10; // target works per Vol (soft cap)
const MIN_GROUP_SIZE = 8; // a Vol should hold at least this many works
...
// The "prefix" of a gallery is the first word before the first space or
// special symbol (Latin letters, digits, or CJK), lower-cased. Same-prefix
// works are treated as one series.
export function galleryPrefix(title) {
  const match = String(title ?? "").trim().match(/^[\p{L}\p{N}]+/u);
  return (match ? match[0] : "").toLowerCase();
}

// Order galleries so that same-prefix works always stay together, and a
// prefix group is ranked by its newest member. A brand-new gallery has the
// max timestamp, so its whole prefix family is pulled to the front with it.
// The prefix comes from the raw folder name (originalTitle), not the cleaned/
// translated display title, so a series groups consistently even when only
// some of its works are translated.
export function sortGalleriesByPrefixGroup(galleries) {
  const rankByPrefix = new Map();
  const decorated = galleries.map((gallery) => {
    const prefix = galleryPrefix(gallery.originalTitle || gallery.title);
    const current = rankByPrefix.get(prefix);
    if (current === undefined || gallery.dirCreatedAt > current) {
      rankByPrefix.set(prefix, gallery.dirCreatedAt);
    }
    return { gallery, prefix };
  });

  decorated.sort((a, b) => {
    const rankDelta = rankByPrefix.get(b.prefix) - rankByPrefix.get(a.prefix);
    if (rankDelta !== 0) {
      return rankDelta; // newest prefix-group first
    }
    if (a.prefix !== b.prefix) {
      return a.prefix < b.prefix ? -1 : 1; // tie on rank: keep each prefix contiguous
    }
    return b.gallery.dirCreatedAt - a.gallery.dirCreatedAt; // within group: newest first
  });

  return decorated.map((entry) => entry.gallery);
}
...
async function buildFlatGalleryRecord(sourceRoot, galleryDirectoryName, dirCreatedAt) {
  ...
  const stableId = await readWorkId(absoluteDirectory);
...
async function readFlatGalleryDirectories(sourceRoot, layoutHints) {
  ...
  return sortGalleriesByPrefixGroup(galleries);
}
...
async function readNestedGalleryDirectories(sourceRoot, rootDirectories) {
  ...
  return sortGalleriesByPrefixGroup(allGalleries);
}
...
export function invalidateGalleryCache() {
  ...
  // Keep the per-directory incremental caches (layout / flatCache / artistCaches)
  // so the next load does a cheap incremental rescan — re-stat the directories
  // and rebuild only the ones whose mtime changed — instead of re-statting every
  // image (~2s on a large external drive). Clearing the assembled list + the
  // freshness window still makes adds/removes/likes show up on the next read,
  // so cross-process consistency is unchanged; only the cost drops.
  galleryCache = {
    ...galleryCache,
    expiresAt: 0,
    items: [],
...
// One newest work per character (same-prefix run), in prefix-group ranked
// order, up to `limit`. Used for cover collages that show a character once.
export function pickCharacterCovers(galleries, limit = Infinity) {
  const ordered = sortGalleriesByPrefixGroup(galleries);
  const covers = [];
  let lastPrefix = null;

  for (const gallery of ordered) {
    const prefix = galleryPrefix(gallery.originalTitle || gallery.title);
...
// Group works into "Vol." blocks. Works are first ranked by prefix group, then
// segmented into characters (same-prefix runs). Characters are packed into Vols
// targeting GROUP_SIZE works each, but a character is never split across Vols —
// so a Vol may exceed GROUP_SIZE when a single character is larger, or to keep a
// straddling character whole. Each Vol's cover collage shows one thumbnail per
// character (its newest work), so the card never shows more than GROUP_SIZE
// thumbnails even when the Vol holds more works.
export function groupGallerySummaries(galleries) {
  const ordered = sortGalleriesByPrefixGroup(galleries);

  // Segment the ranked list into consecutive same-prefix character runs.
  const characters = [];
  let currentPrefix = null;
  for (const gallery of ordered) {
    const prefix = galleryPrefix(gallery.originalTitle || gallery.title);
    if (characters.length === 0 || prefix !== currentPrefix) {
      characters.push([]);
      currentPrefix = prefix;
    }
    characters[characters.length - 1].push(gallery);
  }

  // Pack character runs into Vols, never splitting a character. Aim for
  // GROUP_SIZE works but keep each Vol at least MIN_GROUP_SIZE: only close a Vol
  // once it has met the minimum AND the next character would push it past the
  // target. While still under the minimum, keep adding (even past the target)
  // rather than leave a too-small Vol.
  const blocks = [];
  let current = [];
  let currentWorkCount = 0;
  for (const run of characters) {
    if (
      current.length > 0 &&
      currentWorkCount >= MIN_GROUP_SIZE &&
      currentWorkCount + run.length > GROUP_SIZE
    ) {
      blocks.push(current);
      current = [];
      currentWorkCount = 0;
    ...
    current.push(run);
    currentWorkCount += run.length;
  }
  if (current.length > 0) {
    blocks.push(current);
  }

  // Leftover characters can leave the final block below the minimum; fold it
  // into the previous block rather than show a tiny Vol. (Nothing to do when
  // there is only one block — the whole set is just small.)
  if (blocks.length >= 2) {
    const tailWorks = blocks[blocks.length - 1].reduce((total, run) => total + run.length, 0);
    if (tailWorks < MIN_GROUP_SIZE) {
      const tail = blocks.pop();
      blocks[blocks.length - 1].push(...tail);
    }
  }

  return blocks.map((runs, groupIndex) => {
    const works = runs.flat();
    return {
      slug: toGroupSlug(groupIndex),
      title: `Vol. ${String(groupIndex + 1).padStart(2, "0")}`,
      galleryCount: works.length,
      characterCount: runs.length,
      imageCount: works.reduce((total, gallery) => total + gallery.imageCount, 0),
      galleries: works, // all works (drill-in)
      coverGalleries: runs.map((run) => run[0]), // newest per character (collage)
    };
  });
}
...
// Favorites: galleries whose stable id is liked. The prefix-group ranking is
// computed within the favorites set only (not inherited from the global list),
// so a new work elsewhere doesn't reshuffle the favorites tab.
// Keying by stableId means a like survives the work being renamed or moved.
export async function getLikedGallerySummaries() {
  const liked = await getLikedIdSet();
  ...
  const galleries = await loadGalleries();
  const likedSummaries = galleries
    .filter((gallery) => gallery.stableId && liked.has(gallery.stableId))
    .map(toGallerySummary);

  return sortGalleriesByPrefixGroup(likedSummaries);
}

// Current slugs of all liked galleries — the like is stored by stableId, but
// the UI keys by slug, so we resolve ids back to whatever path they live at now.
export async function getLikedCurrentSlugs() {
  const summaries = await getLikedGallerySummaries();
  return summaries.map((gallery) => gallery.slug);
}

// Toggle a like for the work behind `slug`. Liking ensures the work has a
// stable id marker (creating it on first like); the like is stored by that id.
export async function likeGalleryBySlug(slug, liked) {
  ...
  let id = gallery.stableId;

  if (liked) {
    id = await ensureWorkId(gallery.sourcePath);
  } else if (!id) {
    id = await readWorkId(gallery.sourcePath);
  }

  // Reflect the id in the cached record so favorites/GET see it immediately,
  // before the next full rescan picks the marker file up from disk.
  if (id) {
    gallery.stableId = id;
  }

  if (!id) {
    // No marker and nothing to unlike — treat as a no-op.
    return { slug: gallery.slug, liked: false, id: null };
  }

  await setLikeId(id, liked);

  // Liking writes a marker file (changing the work dir's mtime) and updates the
  // shared like store. This process already reflected the new id on its cached
  // record above, but other server instances still hold a pre-marker cache, so
  // bump the cross-instance invalidation token to make them rescan and read it.
  // (We intentionally do not invalidate this instance's cache — the in-place
  // update above keeps it correct without forcing a full rescan here.)
  await writeGalleryInvalidationToken();

  return { slug: gallery.slug, liked, id };
}

export async function getFavoriteGroups() {
  const summaries = await getLikedGallerySummaries();
  return groupGallerySummaries(summaries);
}

export async function getFavoriteGroupBySlug(slug) {
...
export async function getArtistSummaries() {
  ...
  for (const gallery of galleries) {
    ...
    current.works.push(gallery);
  }

  return Array.from(artists.values())
    .map(({ works, ...artist }) => ({
      ...artist,
      // Preview covers: this artist's first 4 characters (own ranking), each
      // shown as that character's newest work's cover.
      previewGalleries: pickCharacterCovers(works, 4).map(toGallerySummary),
    }))
    .sort((left, right) => right.latestDirCreatedAt - left.latestDirCreatedAt);
}
...
export async function getArtistBySlug(slug) {
  ...
  const galleries = await getGallerySummaries();
  // Rank within this artist's own works only — the global prefix-group order is
  // irrelevant here, so another artist's new work can't pull this artist's old
  // same-prefix work to the front.
  const artistGalleries = sortGalleriesByPrefixGroup(
    galleries.filter((gallery) => gallery.artistSlug === slug),
  );
...
function toNavigationWorks(galleries) {
...
// Resolve the collection page a reader arrived from (its own path) into that
// collection's ordered works, title, and the path to link back to. Only known
// collection routes are honored; anything else returns null so the caller falls
// back to the global grouping. This is what makes a work opened from an artist's
// collection show that artist's works in the strip, not the global "all" block.
async function resolveCollectionContext(fromPath) {
  if (typeof fromPath !== "string" || !fromPath.startsWith("/")) {
    return null;
  }

  const cleanPath = fromPath.split(/[?#]/)[0];
  const segments = cleanPath.split("/").filter(Boolean).map(decodeURIComponent);

  if (segments.length === 2 && segments[0] === "groups") {
    const group = await getGalleryGroupBySlug(segments[1]);
    return group
      ? { works: group.galleries, title: group.title, backHref: cleanPath }
      : null;
  }

  if (segments.length === 2 && segments[0] === "favorites") {
    const group = await getFavoriteGroupBySlug(segments[1]);
    return group
      ? { works: group.galleries, title: group.title, backHref: cleanPath }
      : null;
  }

  if (segments.length === 3 && segments[0] === "artists") {
    const group = await getArtistGroupBySlug(segments[1], segments[2]);
    return group
      ? { works: group.galleries, title: group.title, backHref: cleanPath }
      : null;
  }

  if (segments.length === 2 && segments[0] === "artists") {
    const artist = await getArtistBySlug(segments[1]);
    return artist
      ? { works: artist.galleries, title: artist.title, backHref: cleanPath }
      : null;
  }
...
// The sibling works that share a work's collection. Prefers the collection the
// reader actually came from (`fromPath`, a collection page's own path) so the
// prev/next strip and the back links match that context — an artist's set,
// favorites, or a specific Vol. Falls back to the global Vol. grouping when no
// valid context is supplied. Returns the works, the current position, and the
// collection's title + back href.
export async function getGalleryNavigation(slug, fromPath) {
  const context = await resolveCollectionContext(fromPath);

  if (context) {
    const currentIndex = context.works.findIndex(
      (gallery) => gallery.slug === slug,
    );

    if (currentIndex !== -1) {
      return {
        collectionHref: context.backHref,
        collectionTitle: context.title,
        currentIndex,
        works: toNavigationWorks(context.works),
      };
    }
  }
  ...
  for (const group of groups) {
    const currentIndex = group.galleries.findIndex(
      (gallery) => gallery.slug === slug,
    );

    if (currentIndex !== -1) {
      return {
        collectionHref: `/groups/${group.slug}`,
        collectionTitle: group.title,
        currentIndex,
        works: toNavigationWorks(group.galleries),
      };
    }
  }
