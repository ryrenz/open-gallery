...
// The sibling works that share a work's Vol. block (the global grouping, so a
// work has one canonical collection regardless of how the reader arrived). Used
// to render the prev/next + thumbnail strip at the bottom of a work. Returns the
// full works list plus the current position so the client can derive neighbors.
export async function getGalleryNavigation(slug) {
  const groups = await getGalleryGroups();
  ...
  for (const group of groups) {
    ...
    if (currentIndex !== -1) {
      return {
        ...
        currentIndex,
        works: group.galleries.map((gallery) => ({
          slug: gallery.slug,
          title: gallery.title,
          coverIndex: gallery.coverIndex,
          imageCount: gallery.imageCount,
