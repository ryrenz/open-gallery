...
export function groupGallerySummaries(galleries) {
  ...
  // Pack character runs into Vols, ~GROUP_SIZE works each, never splitting a
  // character. Overflow past GROUP_SIZE only when unavoidable (a single large
  // character, or a character that would straddle the boundary).
  const blocks = [];
  let current = [];
  let currentWorkCount = 0;
  for (const run of characters) {
    if (current.length > 0 && currentWorkCount + run.length > GROUP_SIZE) {
      blocks.push(current);
      current = [];
      currentWorkCount = 0;
    }
